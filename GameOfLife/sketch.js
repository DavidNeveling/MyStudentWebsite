...
function setup(){
    ...
    sim.label = "PLAY";
	 sim.position(0, height);
    sim.mousePressed(advancePlay);

    clear = createButton('CLEAR');
	 clear.position(0, height + sim.height);
    clear.mousePressed(clearGrid);

    randomize = createButton('RANDOMIZE');
	 randomize.position(0, height + sim.height + clear.height);
    ...
    speedSlider = createSlider(0, 999, 999);
	 speedSlider.position(0, height + sim.height + clear.height + randomize.height);
